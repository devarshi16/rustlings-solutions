...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    //map.values().fold(0,|_acc,prog| if value == *prog {count+=1;count} else{count})
    map.iter().fold(0,|count,(_,val)| if val == &value {count+1} else{count})
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    //               { "variables2": Complete, … }, … ]
    collection.iter().fold(0,
        |count,map| count+map.iter().fold(0,
            |acc,(_,val)| if val == &value {acc+1} else{acc}
            )
        )
    //collection.iter().fold(0,
    //    |count,map| count+count_iterator(map,value)
    //)
}
